...
const config = () => {
  ...
  const set = (userConfig = {}) => {
    if (typeof userConfig !== "object" || userConfig.constructor !== Object) {
      throw new Error("the provided Log Eagle config is not a valid object");
    }

    if (
      !userConfig.ticket ||
      typeof userConfig.ticket !== "string" ||
      userConfig.ticket.length !== 50
    ) {
      throw new Error("the provided Log Eagle ticket is invalid");
    }

    if (userConfig.badges && userConfig.badges.constructor !== Object) {
      throw new Error("the provided Log Eagle badges need to be an object");
    }
   ...
   */
  const setSendAnalytics = (sendAnalytics) =>
    (_config = { ..._config, sendAnalytics });
